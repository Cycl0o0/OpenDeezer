...
cross:
	@mkdir -p $(DIST)
	CGO_ENABLED=0 GOOS=darwin  GOARCH=arm64 go build -trimpath -ldflags "$(LDFLAGS)" -o $(DIST)/$(BIN)-darwin-arm64  $(PKG)
	CGO_ENABLED=0 GOOS=darwin  GOARCH=amd64 go build -trimpath -ldflags "$(LDFLAGS)" -o $(DIST)/$(BIN)-darwin-amd64  $(PKG)
	@command -v x86_64-w64-mingw32-gcc >/dev/null 2>&1 || \
		{ echo "error: windows needs cgo (malgo audio); install mingw-w64 for x86_64-w64-mingw32-gcc" >&2; exit 1; }
	CGO_ENABLED=1 CC=x86_64-w64-mingw32-gcc GOOS=windows GOARCH=amd64 \
		go build -trimpath -ldflags "$(LDFLAGS) -extldflags=-static" -o $(DIST)/$(BIN)-windows-amd64.exe $(PKG)
	@echo "built: $(DIST)/"
