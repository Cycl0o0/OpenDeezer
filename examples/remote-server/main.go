...
func main() {
	...
	//
	// The control server invokes the status snapshot and the command callbacks
	// from independent HTTP handler goroutines, so every access to shared state
	// must be synchronized; mu guards `state`.
	var mu sync.Mutex
	state := control.State{
		State:  "stopped",
		Volume: 1.0,
